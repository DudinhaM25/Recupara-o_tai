...
class BD
{
    ...
    {
        $conn = $this->conn();
        $sql = "INSERT contato (nome, sobrenome, telefone01, tipo_telefone01, telefone02, tipo_telefone02, email) value (?, ?, ?, ?, ?, ?, ?,?)";
    ...
    {
        $conn = $this->conn();
        $sql = "UPDATE contato SET nome= ?, sobrenome= ?, telefone01= ?, tipo_telefone01= ?, telefone02= ?,tipo_telefone02= ?,email= ? WHERE id = ?";

        $st = $conn->prepare($sql);
        $arrayDados = [$dados['nome'], $dados['sobrenome'], $dados['telefone01'], $dados['tipo_telefone01'], $dados['telefone02'],
        $dados['tipo_telefone02'],$dados['email'], $dados['id']];
        $st->execute($arrayDados);
